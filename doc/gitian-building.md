...
*Setup instructions for a Gitian build of Iustitiacoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Iustitiacoin
...
- [Building Iustitiacoin Core](#building-iustitiacoin-core)
...
Clone the git repositories for iustitiacoin and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/iustitiacoin-project/iustitiacoin
...
in the iustitiacoin repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Iustitiacoin Core
----------------

To build Iustitiacoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the iustitiacoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/iustitiacoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/iustitiacoin-project/iustitiacoin
...
For example:
```bash
URL=https://github.com/thrasher-/iustitiacoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit iustitiacoin=${COMMIT} --url iustitiacoin=${URL} ../iustitiacoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit iustitiacoin=${COMMIT} --url iustitiacoin=${URL} ../iustitiacoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit iustitiacoin=${COMMIT} --url iustitiacoin=${URL} ../iustitiacoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the iustitiacoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../iustitiacoin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/iustitiacoin
SIGPATH=/some/root/path/iustitiacoin-detached-sigs

./bin/gbuild --url iustitiacoin=${BTCPATH},signature=${SIGPATH} ../iustitiacoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/iustitiacoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/iustitiacoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/iustitiacoin-osx-build.assert
```
...
[iustitiacoin-project/gitian.sigs.ius](https://github.com/iustitiacoin-project/gitian.sigs.ius/) repository, or if that's not possible create a pull
