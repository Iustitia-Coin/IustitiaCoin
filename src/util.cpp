...
const char * const BITCOIN_CONF_FILENAME = "iustitiacoin.conf";
const char * const BITCOIN_PID_FILENAME = "iustitiacoin.pid";
...
#else
    const char* pszModule = "iustitiacoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Iustitiacoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Iustitiacoin";
#else
    // Unix
    return pathRet / ".iustitiacoin";
#endif
#endif
}
